...
_Backup an Othent account with a JWK private key. It receives a `{ JWK_public_key_PEM }` object, the JWK PUBLIC key in PEM format!_

```javascript

// Initalize JWK to user

const transaction = await othent.initializeJWK({ JWK_public_key_PEM })
...
// Note this function is only available in node, unless you use next.js or pollyfill the package from othent/node
